...
func TestDoubleLiteralExpression(t *testing.T) {
	input := "5;"
	...
	if !ok {
		t.Fatalf("exp is not *ast.DoubleLiteral. got=%T", stmt.Expression)
	}
	if literal.Value != 5 {
		t.Errorf("literal.Value is not %d. got=%f", 5, literal.Value)
	}
